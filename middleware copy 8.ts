...
export function middleware(request: NextRequest) {
    ...
    if (isAuthenticated) {
        ...
        //     console.log("middleware - abc - both for specific and nested path");
        //     return NextResponse.redirect(new URL('/hello-0', request.url));
        // }

        if (request.nextUrl.pathname.startsWith('/abc') && /*only for specific, not for nested path*/ request.nextUrl.pathname === '/abc') {
            console.log("middleware - authenticated - abc - only for specific without nested path");
            return NextResponse.redirect(new URL('/hello-1', request.url));
        }

        if (request.nextUrl.pathname.startsWith('/abc') && /*not for specific, only for nested path*/ request.nextUrl.pathname !== '/abc') {
            console.log("middleware - authenticated - abc - not only for specific with nested path");
            return NextResponse.redirect(new URL('/hello-2', request.url));
        }
